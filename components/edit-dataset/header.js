...
export default function PageHeader({ dataset_id, tabs, onTabChange }) {
    return (
            ...
            <Space direction='vertical' style={{ display: 'flex' }}>
                <Breadcrumb 
                    items={[
                        ...
                        {title: <Link href={`/my-dataset/${dataset_id}`}>{dataset_id}</Link>},
                        {title: 'Edit Dataset'}
                    ]}
                />
                <Title level={2} style={{ margin: 0 }}>{`Edit Dataset (${dataset_id})`}</Title>
                <Paragraph />
